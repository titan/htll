          ...
          (if (or (= 0 ratio-or-margin) (> ratio-or-margin 1))
              (let ((margin ratio-or-margin))
                (let* ((frame1 (layout root up x y width 0))
                       (frame2 (layout root low x `(+ (+ ,y ,(cadddr frame1)) ,margin) width `(- ,height (+ (+ ,y ,(cadddr frame1)) ,margin)))))
          ...
          (if (or (= 0 ratio-or-margin) (> ratio-or-margin 1))
              (let ((margin ratio-or-margin))
                (let* ((frame1 (layout root left x y 0 height))
                       (frame2 (layout root right `(+ (+ ,x ,(caddr frame1)) ,margin) y `(- ,width (+ (+ ,x ,(caddr frame1)) ,margin)) height)))
                  (list x y width height)))
              (let ((ratio ratio-or-margin))
