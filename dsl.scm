           ...
           (let ((aop (car a))
             ...
             (if (eq? aop bop)
                 (let ((aa (cadr a))
                       ...
                       (bb (caddr b)))
                   (cond
                    ((and (eq? aop '+) (or (exp-eq? aa ba) (exp-eq? aa bb)) (or (exp-eq? ab ba) (exp-eq? ab bb))) #t)
                    ((and (eq? aop '*) (or (exp-eq? aa ba) (exp-eq? aa bb)) (or (exp-eq? ab ba) (exp-eq? ab bb))) #t)
                    ((and (exp-eq? aa ba) (exp-eq? ab bb)) #t)
                    (else #f)))
                 #f)))
          (else #f)))
  (define (combine exp)
    (define (b++ a b c) ; (+ (+ a b) c)
      (cond ((exp-eq? a c) (exp-optimizer `(+ (* 2 ,a) ,b)))
            ((exp-eq? b c) (exp-optimizer `(+ (* 2 ,b) ,a)))
    ...
    (define (b+- a b c) ; (+ (- a b) c)
      (cond ((exp-eq? a c) (exp-optimizer `(- (* 2 ,a) ,b)))
            ...
            (else `(+ (- ,a ,b) ,c))))
    (define (b+* a b c) ; (+ (* a b) c)
      (cond ((exp-eq? a c) (exp-optimizer `(* (+ 1 ,b) ,a)))
            ((exp-eq? b c) (exp-optimizer `(* (+ 1 ,a) ,b)))
            (else `(+ (* ,a ,b) ,c))))
    (define (b-+ a b c) ; (- (+ a b) c)
      (cond ((exp-eq? a c) b)
            ((exp-eq? b c) a)
            ...
            (else `(- (+ ,a ,b) ,c))))
    (define (b-- a b c) ; (- (- a b) c)
      (cond ((exp-eq? a c) (exp-optimizer `(- 0 ,b)))
            ((exp-eq? b c) (exp-optimizer `(- ,a (* 2 ,b))))
            ((and (number? a) (number? c)) (exp-optimizer `(- ,(- a c) ,b)))
            ((and (number? b) (number? c)) (exp-optimizer `(- ,a ,(+ b c))))
            (else `(- (- ,a ,b) ,c))))
    (define (b-* a b c) ; (- (* a b) c)
      (cond ((exp-eq? a c) (exp-optimizer `(* ,a (- ,b 1))))
            ((exp-eq? b c) (exp-optimizer `(* ,b (- ,a 1))))
            (else `(- (* ,a ,b) ,c))))
    (define (b** a b c) ; (* (* a b) c)
    ...
    (define (a++ a b c) ; (+ a (+ b c))
      (cond ((exp-eq? a b) (exp-optimizer `(+ (* 2 ,a) ,c)))
            ((exp-eq? a c) (exp-optimizer `(+ (* 2 ,a) ,b)))
    ...
    (define (a+- a b c) ; (+ a (- b c))
      (cond ((exp-eq? a b) (exp-optimizer `(- (* 2 ,a) ,c)))
            ...
            (else `(+ ,a (- ,b ,c)))))
    (define (a+* a b c) ; (+ a (* b c))
      (cond ((exp-eq? a b) (exp-optimizer `(* (+ 1 ,c) ,a)))
            ((exp-eq? a c) (exp-optimizer `(* (+ 1 ,b) ,a)))
            (else `(+ ,a (* ,b ,c)))))
    (define (a-+ a b c) ; (- a (+ b c))
      (cond ((exp-eq? a b) (exp-optimizer `(- 0 ,c)))
            ((exp-eq? a c) (exp-optimizer `(- 0 ,b)))
            ((and (number? a) (number? b)) (exp-optimizer `(- ,(- a b) ,c)))
            ((and (number? a) (number? c)) (exp-optimizer `(- ,(- a c) ,b)))
            (else `(- ,a (+ ,b ,c)))))
    (define (a-- a b c) ; (- a (- b c))
    ...
    (define (+++ a b c d) ; (+ (+ a b) (+ c d))
      (cond ((exp-eq? a c) (exp-optimizer `(+ (* 2 ,a) (+ ,b ,d))))
            ((exp-eq? a d) (exp-optimizer `(+ (* 2 ,a) (+ ,b ,c))))
            ((exp-eq? b c) (exp-optimizer `(+ (* 2 ,b) (+ ,a ,d))))
            ((exp-eq? b d) (exp-optimizer `(+ (* 2 ,b) (+ ,a ,c))))
    ...
    (define (++- a b c d) ; (+ (+ a b) (- c d))
      (cond ((exp-eq? a c) (exp-optimizer `(+ (* 2 ,a) (- ,b ,d))))
            ((exp-eq? a d) (exp-optimizer `(+ ,b ,c)))
            ((exp-eq? b c) (exp-optimizer `(+ (* 2 ,b) (- ,c ,d))))
            ((exp-eq? b d) (exp-optimizer `(+ ,a ,c)))
            ((and (number? a) (number? c)) (exp-optimizer `(+ ,(+ a c) (- ,b ,d))))
            ((and (number? a) (number? d)) (exp-optimizer `(+ ,(- a d) (+ ,b ,c))))
            ((and (number? b) (number? c)) (exp-optimizer `(+ ,(+ b c) (- ,a ,d))))
            ((and (number? b) (number? d)) (exp-optimizer `(+ ,(- b d) (+ ,a ,c))))
            (else `(+ (+ ,a ,b) (- ,c ,d)))))
    (define (+-+ a b c d) ; (+ (- a b) (+ c d))
      (cond ((exp-eq? a c) (exp-optimizer `(+ (* 2 ,a) (- ,d ,b))))
            ((exp-eq? a d) (exp-optimizer `(+ (* 2 ,a) (- ,c ,b))))
            ((exp-eq? b c) (exp-optimizer `(+ ,a ,d)))
            ((exp-eq? b d) (exp-optimizer `(+ ,a ,c)))
    ...
    (define (+-- a b c d) ; (+ (- a b) (- c d))
      (cond ((exp-eq? a c) (exp-optimizer `(- (* 2 ,a) (+ ,b ,d))))
            ((exp-eq? a d) (exp-optimizer `(- ,c ,b)))
            ((exp-eq? b c) (exp-optimizer `(- ,a ,d)))
            ((exp-eq? b d) (exp-optimizer `(- (+ ,a ,c) (* 2 ,b))))
            ((and (number? a) (number? c)) (exp-optimizer `(- ,(+ a c) (+ ,b ,d))))
            ((and (number? a) (number? d)) (exp-optimizer `(+ ,(- a d) (- ,c ,b))))
            ((and (number? b) (number? c)) (exp-optimizer `(+ ,(- c b) (- ,a ,d))))
    ...
    (define (+** a b c d) ; (+ (* a b) (* c d))
      (cond ((and (number? a) (number? c) (symbol? b) (symbol? d) (eq? b d)) (exp-optimizer `(* ,(+ a c) ,b)))
            ...
            ((exp-eq? a c) (exp-optimizer `(* ,a (+ ,b ,d))))
            ((exp-eq? a d) (exp-optimizer `(* ,a (+ ,b ,c))))
            ((exp-eq? b c) (exp-optimizer `(* ,b (+ ,a ,d))))
            ((exp-eq? b d) (exp-optimizer `(* ,b (+ ,a ,c))))
            (else `(+ (* ,a ,b) (* ,c ,d)))))
    (define (-++ a b c d) ; (- (+ a b) (+ c d))
      (cond ((exp-eq? a c) (exp-optimizer `(- ,b ,d)))
            ((exp-eq? a d) (exp-optimizer `(- ,b ,c)))
            ((exp-eq? b c) (exp-optimizer `(- ,a ,d)))
            ((exp-eq? b d) (exp-optimizer `(- ,a ,c)))
            ((and (number? a) (number? c)) (exp-optimizer `(+ ,(- a c) (- ,b ,d))))
    ...
    (define (-+- a b c d) ; (- (+ a b) (- c d))
      (cond ((exp-eq? a c) (exp-optimizer `(+ ,b ,d)))
            ...
            (else `(- (+ ,a ,b) (- ,c ,d)))))
    (define (--+ a b c d) ; (- (- a b) (+ c d))
      (cond ((exp-eq? a c) (exp-optimizer `(- 0 (+ ,b ,d))))
            ((exp-eq? a d) (exp-optimizer `(- 0 (+ ,b ,c))))
            ((exp-eq? b c) (exp-optimizer `(- (- ,a ,d) (* 2 ,b))))
            ((exp-eq? b d) (exp-optimizer `(- (- ,a ,c) (* 2 ,b))))
            ((and (number? a) (number? c)) (exp-optimizer `(- ,(- a c) (+ ,b ,d))))
            ((and (number? a) (number? d)) (exp-optimizer `(- ,(- a d) (+ ,b ,c))))
            ...
            (else `(- (- ,a ,b) (+ ,c ,d)))))
    (define (--- a b c d) ; (- (- a b) (- c d))
      (cond ((exp-eq? a c) (exp-optimizer `(- ,d ,b)))
            ((exp-eq? a d) (exp-optimizer `(- (* 2 ,a) (+ ,b ,c))))
            ((exp-eq? b c) (exp-optimizer `(- (+ ,a ,d) (* 2 ,b))))
            ((exp-eq? b d) (exp-optimizer `(- ,a ,c)))
            ((and (number? a) (number? c)) (exp-optimizer `(- ,(- a c) (- ,b ,d))))
  ...
  (let ((exp (exp-optimizer exp)))
    (cond
     ...
     ((tagged-list? exp '/)
      (string-append (layout-eval (cadr exp) 2) " / " (layout-eval (caddr exp) 2)))
     ((tagged-list? exp 'max)
      (string-append "MAX(" (layout-eval (cadr exp) 0) ", " (layout-eval (caddr exp) 0) ")"))
...
(define (layout-view root view x y width height gen?)
  ...
  (define (layout-^^ root view x y width height gen?)
    (let ((up (up-view view))
      ...
      (let* ((occupy1 (layout-view root up x y width 0 gen?))
             (occupy2 (layout-view root low x `(+ ,y (+ ,margin ,(frame-height occupy1))) width (if (and (number? height) (= 0 height)) 0 `(- ,height (+ ,margin ,(frame-height occupy1)))) gen?)))
        (let ((w1 (frame-width occupy1))
              (h1 (frame-height occupy1))
              (w2 (frame-width occupy2))
              (h2 (frame-height occupy2)))
          (list x y (if (and (number? width) (= 0 width)) (cond ((and (number? w1) (number? w2)) (max w1 w2)) ((and (number? w1) (= 0 w1)) w2) ((and (number? w2) (= 0 w2)) w1) (else `(max ,w1 ,w2)))) `(+ (+ ,h1 ,h2) ,margin))))))
  (define (layout-vv root view x y width height gen?)
    (let ((up (up-view view))
          (low (low-view view))
          (margin (view-margin view)))
      (let* ((occupy0 (layout-view root low x y width 0 #f))
             (occupy1 (layout-view root up x y width (if (and (number? height) (= 0 height)) 0 `(- ,height (+ ,margin ,(frame-height occupy0)))) gen?))
             (occupy2 (layout-view root low x `(+ ,y (+ ,margin ,(frame-height occupy1))) width (if (and (number? height) (= 0 height)) 0 `(- ,height (+ ,margin ,(frame-height occupy1)))) gen?)))
        (let ((w1 (frame-width occupy1))
              (h1 (frame-height occupy1))
              (w2 (frame-width occupy2))
  ...
  (define (layout-<< root view x y width height gen?)
    (let ((left (left-view view))
          ...
          (margin (view-margin view)))
      (let* ((occupy1 (layout-view root left x y 0 height gen?))
             (occupy2 (layout-view root right `(+ ,x (+ ,margin ,(frame-width occupy1))) y (if (and (number? width) (= 0 width)) 0 `(- ,width (+ ,margin ,(frame-width occupy1)))) height gen?)))
        (let ((w1 (frame-width occupy1))
              (h1 (frame-height occupy1))
              (w2 (frame-width occupy2))
              (h2 (frame-height occupy2)))
          (list x y `(+ (+ ,w1 ,w2) ,margin) (if (and (number? height) (= 0 height)) (cond ((and (number? h1) (number? h2)) (max h1 h2)) ((and (number? h1) (= 0 h1)) h2) ((and (number? h2) (= 0 h2)) h1) (else `(max ,h1 ,h2))) height))))))
  (define (layout->> root view x y width height gen?)
    (let ((left (left-view view))
          (right (right-view view))
          (margin (view-margin view)))
      (let* ((occupy0 (layout-view root right x y 0 height #f))
             (occupy1 (layout-view root left x y (if (and (number? width) (= 0 width)) 0 `(- ,width (+ ,margin ,(frame-width occupy0)))) height gen?))
             (occupy2 (layout-view root right `(+ ,x (+ ,margin ,(frame-width occupy1))) y (if (and (number? width) (= 0 width)) 0 `(- ,width (+ ,margin ,(frame-width occupy1)))) height gen?)))
        (let ((w1 (frame-width occupy1))
              (h1 (frame-height occupy1))
              (w2 (frame-width occupy2))
              (h2 (frame-height occupy2)))
          (list x y `(+ (+ ,w1 ,w2) ,margin) (if (and (number? height) (= 0 height)) (cond ((and (number? h1) (number? h2)) (max h1 h2)) ((and (number? h1) (= 0 h1)) h2) ((and (number? h2) (= 0 h2)) h1) (else `(max ,h1 ,h2))) height))))))
  (define (layout-above root view x y width height gen?)
    (let ((up (up-view view))
          (low (low-view view))
          (ratio (view-ratio view)))
      (if (and (number? height) (= 0 height))
          (let ((occupy1 (layout-view root up x y width 0 #f))
                (occupy2 (layout-view root low x y width 0 #f)))
            (cond
             ...
             ((and (number? (frame-height occupy1)) (= 0 (frame-height occupy1)))
              (layout-view root low x y width (frame-height occupy2) gen?))
             ((and (number? (frame-height occupy2)) (= 0 (frame-height occupy2)))
              (layout-view root up x y width (frame-height occupy1) gen?))
             (else (let ((h `(+ ,(frame-height occupy1) ,(frame-height occupy2))))
                     (layout-view root up x y width (frame-height occupy1) gen?)
                     (layout-view root low x `(+ ,y ,(frame-height occupy1)) width (frame-height occupy2) gen?)
                     (list x y width h)))))
          (let* ((occupy1 (layout-view root up x y width `(* ,height ,ratio) gen?))
                 (occupy2 (layout-view root low x `(+ ,y ,(frame-height occupy1)) width `(* ,height (- 1 ,ratio)) gen?)))
            (list x y width height)))))
  (define (layout-beside root view x y width height gen?)
    (let ((left (left-view view))
          (right (right-view view))
          (ratio (view-ratio view)))
      (if (and (number? width) (= 0 width))
          (let ((occupy1 (layout-view root left x y 0 height #f))
                (occupy2 (layout-view root right x y 0 height #f)))
            (cond
             ...
             ((and (number? (frame-width occupy1)) (= 0 (frame-width occupy1)))
              (layout-view root right x y (frame-width occupy2) height gen?))
             ((and (number? (frame-width occupy2)) (= 0 (frame-width occupy2)))
              (layout-view root left x y (frame-width occupy1) height gen?))
             (else (let ((w `(+ ,(frame-width occupy1) ,(frame-width occupy2))))
                     (layout-view root left x y (frame-width occupy1) height gen?)
                     (layout-view root right `(+ ,x ,(frame-width occupy1)) y (frame-width occupy2) height gen?)
                     (list x y w height)))))
          (let* ((frame1 (layout-view root left x y `(* ,width ,ratio) height gen?))
                 (frame2 (layout-view root right `(+ ,x ,(frame-width frame1)) y `(* ,width (- 1 ,ratio)) height gen?)))
            (list x y width height)))))
  (cond
   ((eq? '^^ (view-type view))
    (layout-^^ root view x y width height gen?))
   ((eq? 'vv (view-type view))
    (layout-vv root view x y width height gen?))
   ((eq? '<< (view-type view))
    (layout-<< root view x y width height gen?))
   ((eq? '>> (view-type view))
    (layout->> root view x y width height gen?))
   ((eq? 'above (view-type view))
    (layout-above root view x y width height gen?))
   ((eq? 'beside (view-type view))
    (layout-beside root view x y width height gen?))
   (else
  ...
  (define (layout-loop tree)
    (if (not (null? tree))
        (let ((root (caar tree))
              ...
              (children (cdar tree)))
          (layout-view root children 0 0 (string->symbol (string-append rootstr ".bounds.size.width")) (string->symbol (string-append rootstr ".bounds.size.height")) #t)
          (layout-loop (cdr tree)))))
  (layout-loop the-layout-tree))

(define (generate-cell-declaration)
  (let loop ((vars the-variables))
    (if (not (null? vars))
        (let ((definition (assq (view-type (cadar vars)) the-type-definitions)))
          (if definition
              (let ((var (caar vars))
                    (type (type-definition-type-string definition)))
                (display (string-append type " * " (symbol->string var) ";"))
                (newline)))
          (loop (cdr vars))))))

(define (generate-cell-view-ref root start-number)
  (let loop ((vars the-variables)
    ...
    (if (not (null? vars))
        (let ((definition (assq (view-type (cadar vars)) the-type-definitions)))
          (if definition
              (let ((var (caar vars))
                    (type (type-definition-type-string definition)))
                (display (string-append (symbol->string var) " = (" type " *)[" root ".contentView viewWithTag:" (number->string (+ 100 counter)) "];"))
                (newline)))
          (loop (cdr vars) (+ counter 1))))))

(define (generate-cell-view-init start-number)
  (define (generate-init type type-string init-fun root var view counter)
    (let ((res (memq type the-resource-types)))
      (if (not res) ; resource types don't need to be add to root view
          (begin
            (display (string-append (symbol->string var) " = " (init-fun type-string view) ";"))
            (newline)
            (display (string-append (symbol->string var) ".tag = " (number->string (+ 100 counter)) ";"))
            (newline)
            (display (string-append "[" (symbol->string root) ".contentView addSubview:" (symbol->string var) "];"))
            (newline)))))
  (let loop ((vars the-variables)
             (counter start-number))
    (if (not (null? vars))
...
(define (generate-cell-init env)
  (let ((tree the-layout-tree))
    (if (not (null? tree))
        (let ((root (caar tree))
              (rootstr (symbol->string (caar tree)))
              (child (cdar tree)))
          (generate-cell-declaration)
          (display "static NSString * CellIdentifier = @\"CellIdentifier\";")
          (newline)
          (display (string-append "UITableViewCell * " rootstr " = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];"))
          (newline)
          (display (string-append "if (" rootstr " == nil) {"))
          (newline)
          (display (string-append rootstr " = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];"))
          (newline)
          (generate-cell-view-init 0)
          (display "} else {")
          (newline)
          (generate-cell-view-ref rootstr 0)
          (display "}")
          (newline)))))

(define (generate-cell-view-layout tree)
  (let ((root (caar tree))
        (rootstr (string-append (symbol->string (caar tree)) ".contentView"))
        (children (cdar tree)))
    (layout-view root children 0 0 (string->symbol (string-append rootstr ".bounds.size.width")) (string->symbol (string-append rootstr ".bounds.size.height")) #t)))

(define (generate-cell-layout env)
  (let ((tree the-layout-tree))
    (if (not (null? tree))
        (let ((root (caar tree))
              (rootstr (symbol->string (caar tree)))
              (child (cdar tree)))
          (generate-cell-view-layout tree)))))
