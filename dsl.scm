    ...
    (if (not (null? vars))
        (let ((definition (assq (view-type (cadar vars)) the-type-definitions)))
          (if definition
              (let ((var (caar vars))
                    (type (type-definition-type-string definition)))
                (display (string-append (symbol->string var) " = (" type " *)[" root ".contentView viewWithTag:" (number->string (+ 100 start-number)) "];"))
                (newline)))
          (loop (cdr vars) (+ counter 1))))))

(define (generate-cell-view-init)
  (define (generate-init type type-string init-fun root var view)
    (let ((res (memq type the-resource-types)))
      (if (not res) ; resource types don't need to be add to root view
          (begin
            (display (string-append (symbol->string var) " = " (init-fun type-string view) ";"))
        ...
        (let ((definition (assq (view-type (cadar vars)) the-type-definitions)))
          (if definition
              (let ((type (type-definition-type definition))
                ...
                (generate-init type type-string init-fun root var view)
                (if generators
